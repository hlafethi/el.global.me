...
**Sous-domaine de test :** `elp.webglobal.me`
...
4. Configurer Nginx Proxy Manager pour exposer `elp.webglobal.me` vers ce conteneur.
...
4. **Port mapping** :
   - **Published port** : `3080` (comme dans le `docker-compose.yml` du projet).
   - **Container port** : `80` (Nginx écoute sur 80 dans le conteneur).

   En résumé : port **3080** sur l’hôte → port **80** dans le conteneur.
...
- Dans un navigateur : `http://VOTRE_IP_VPS:3080`.
...
Cela crée et démarre le conteneur `el-proprete` sur le port **3080**. Le fichier `docker-compose.yml` est déjà dans le projet.
...
## Étape 5 : Configurer Nginx Proxy Manager (NPM) pour elp.webglobal.me

Objectif : faire pointer le sous-domaine **elp.webglobal.me** vers le conteneur (port 3080 sur le VPS).
...
- Créer un enregistrement **A** :
  - **Nom / sous-domaine** : `elp` (pour elp.webglobal.me).
...
2. **Details** :
   ...
   - **Forward Port** : `3080` (ou le port publié du conteneur à l’étape 4).
...
Dans un navigateur : **https://elp.webglobal.me**

- Vous devez voir le site E & L PROPRETE en HTTPS.

Si erreur 502 Bad Gateway : le conteneur n’est pas joignable sur le port indiqué. Vérifier dans Portainer que le conteneur tourne et que le port mappé (3080) est bien celui renseigné dans NPM.
...
| 4 | Créer conteneur dans Portainer (image el-proprete:latest, port 3080→80) | Portainer |
| 5 | DNS : elp.webglobal.me → IP du VPS | Hébergeur DNS |
| 6 | NPM : Proxy Host elp.webglobal.me → 127.0.0.1:3080 + SSL | Nginx Proxy Manager |
...
Le domaine **elp.webglobal.me** continuera de pointer vers le nouveau conteneur.
...
- **SSL ne se crée pas** : vérifier que elp.webglobal.me pointe bien vers l’IP du VPS (ping, ou outil type https://dnschecker.org).
...
Une fois ces étapes suivies, votre site est servi en HTTPS sur **elp.webglobal.me** depuis votre VPS Hostinger avec Docker, Portainer et Nginx Proxy Manager.
